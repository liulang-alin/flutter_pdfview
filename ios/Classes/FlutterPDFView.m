...
static const int PASSWORD_ERROR = 100;
        ...
        if (document == nil) {
            ...
            if ([_pdfView.document isEncrypted]) {
                if ([password isKindOfClass:[NSNull class]]) {
                    // 第一次加载的时候。password为空
                    [_channel invokeMethod:@"onError" arguments:@{@"error": @{@"errorMsg" : @"password needed", @"code": @(PASSWORD_ERROR)}}];
                } else {
                    if (![_pdfView.document unlockWithPassword:password]) {
                        [_channel invokeMethod:@"onError" arguments:@{@"error": @{@"errorMsg" : @"password incorrect", @"code": @(PASSWORD_ERROR)}}];
                    }
                }
            }
